...
import { Bell, BookOpen, BrainCircuit, CheckCircle2, ChevronLeft, ChevronRight, CircleAlert, ClipboardList, GraduationCap, Home, LogOut, Menu, School, Settings2, ShieldCheck, Smartphone, Users, X } from 'lucide-react';
    ...
    <div className="app-shell">
      <aside className={`sidebar ${mobileOpen ? 'mobile-open' : ''}`}>
        <div className="brand-row">
          ...
          <div><strong>HanQuoc</strong><span>Classroom · v2.2.8</span></div>
          <button className="icon-button close-mobile" onClick={() => setMobileOpen(false)} aria-label="Đóng menu"><X size={20} /></button>
        </div>
        <nav className="side-nav">
          {navItems.map(({ to, label, icon: Icon, badge }) => (
            <NavLink key={to} to={to} end={to === '/'} onClick={() => setMobileOpen(false)} className={({ isActive }) => isActive ? 'nav-link active' : 'nav-link'}>
              <Icon size={19} /><span>{label}</span>{badge ? <b className="nav-badge">{badge}</b> : null}
            </NavLink>
          ))}
        </nav>
        <div className="sidebar-bottom">
          {installPrompt && <button className="nav-link install" onClick={install}><Smartphone size={19} /><span>Cài app điện thoại</span></button>}
...
export function StatCard({ label, value, note, tone = 'purple', icon: Icon = ShieldCheck }) {
  return <div className={`stat-card ${tone}`}><div className="stat-icon"><Icon size={20} /></div><div><span>{label}</span><strong>{value}</strong><small>{note}</small></div></div>;
}

export function Empty({ children = 'Chưa có dữ liệu.' }) {
  return <div className="empty-state"><GraduationCap size={30} /><p>{children}</p></div>;
}
...
export function Pagination({ pagination, onPageChange, loading = false, label = 'dữ liệu' }) {
  ...
  return <div className="pagination-bar" aria-label={`Phân trang ${label}`}>
    <span>{pagination.total} {label} · trang {current}/{totalPages}</span>
    <div>
      <button type="button" disabled={loading || !pagination.hasPrevious} onClick={() => onPageChange(current - 1)} aria-label="Trang trước"><ChevronLeft size={16} /></button>
      {pageNumbers.map((page, index) => <span key={page} className="pagination-number-wrap">{index > 0 && page - pageNumbers[index - 1] > 1 ? <i>…</i> : null}<button type="button" disabled={loading} className={page === current ? 'active' : ''} onClick={() => page !== current && onPageChange(page)}>{page}</button></span>)}
      <button type="button" disabled={loading || !pagination.hasNext} onClick={() => onPageChange(current + 1)} aria-label="Trang sau"><ChevronRight size={16} /></button>
    </div>
  </div>;
}
